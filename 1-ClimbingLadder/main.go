...
func climbingLeaderboard(ranked []int32, player []int32) []int32 {
	...
	for i := 0; i < len(ranked); i++ {
		...
		newRanked[i-dup] = ranked[i]
	}
	newRanked = newRanked[:len(ranked)-dup]

	i := len(newRanked) - 1
	lasti := i
	for pi := 0; pi < len(player); pi++ {
		i = lasti
		for {
			if player[pi] == newRanked[i] {
				result[pi] = int32(i + 1)
				lasti = i
				break
			}
			if player[pi] < newRanked[i] {
				result[pi] = int32(i+1) + 1
				lasti = i
				break
			}
			if player[pi] > newRanked[0] {
				result[pi] = 1
				break
			}
			if i > 0 && player[pi] > newRanked[i] {
				i--
			}
		}
	}
